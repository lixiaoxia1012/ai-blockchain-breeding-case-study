...
def run_scenario(epi_scale, n_rep=10):
    ...
    out = {k: (np.nanmean(v), np.nanstd(v) / np.sqrt(n_rep),
               np.mean(rmse[k]), np.std(rmse[k]) / np.sqrt(n_rep)) for k, v in rec.items()}
    return out, rec
...
ax.text(0.97, 0.94, 'DNN advantage grows with training size;\nwithin-population sweep: Supplementary Table S2',
